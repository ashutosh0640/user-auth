...
public class ProfileServiceImpl implements ProfileService {
	...
	public List<ProfileDTO> saveAll(Iterable<Profile> profiles) {
		try {
			...
			return profileRepo.saveAll(profiles).stream().map(ProfileMapper::toDTO).collect(Collectors.toList());
		} catch (Exception ex) {
	...
	public List<ProfileDTO> findAll() {
		try {
			...
			return profileRepo.findAll().stream().map(ProfileMapper::toDTO).collect(Collectors.toList());
		} catch (Exception ex) {
	...
	public List<ProfileDTO> findAllById(Iterable<Long> ids) {
		try {
			...
			return profileRepo.findAllById(ids).stream().map(ProfileMapper::toDTO).collect(Collectors.toList());
		} catch (Exception ex) {
	...
	public ProfileDTO updateProfile(Long id, Profile newProfile) {
		try {
			...
			Profile existingProfile = profileRepo.findById(id)
					.orElseThrow(() -> new EntityNotFoundException("Profile not found with ID: " + id));
			if (username == null || username != existingProfile.getUser().getUsername()) {
